...
// Settings for the widget:pick2 element, ported verbatim (request/response handling and
// endpoints) from src/app/channel/[id]/widgets/page.tsx. Save pushes a `pick2` spine refetch
// cue via useSettingWrite.ts so OBS sees the new price immediately rather than waiting for the
// spine's 5s poll.
...
export default function Pick2Settings({channelId, onFireCue}: {
    ...
    async function save() {
        await writeSetting('pick2', () => post(getEndpoints().widget_pick2_update, {channel_id: channelId, price}))
    }
