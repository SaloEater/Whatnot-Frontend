...
import {
    ...
    ObsItem,
    ObsScene, RawObsItem, SimpleAnimations,
    TeamAnimation,
    TeamAnimations,
    TeamLogos
...
import {filterOnlyType, RawObsItemTypeImage, RawObsItemTypeMedia} from "@/app/utils/obs_item_utils";
...
export const TeamsComponent: FC<TeamsProps> = (props) => {
    const [animations, setAnimations] = useState<TeamAnimations>({sceneName: EmptyName, animations: []})
    const [teamLogos, setTeamLogos] = useState<TeamLogos>({sceneName: EmptyName, logos: []})
    const [simpleAnimations, setSimpleAnimations] = useState<SimpleAnimations>({sceneName: EmptyName, animations: []})
    ...
    function createAnimations() {
        let emptyAnimations: TeamAnimations = {sceneName: EmptyName, animations: []}
        Teams.forEach((i, j) => {
    ...
    function createSimpleAnimations() {
        let simpleAnimationsEmpty: SimpleAnimations = {sceneName: EmptyName, animations: []}
    ...
    return <div>
        {
            props.obs.isConnected() && <SceneComponent obs={props.obs} logger={props.logger} setScene={setObsScene} scene={obsScene}/>
        }
        {obsScene && <StepsComponent steps={[
            {
                name: 'Team Logo',
                node: <SetupLogosComponent updateTeam={updateTeamLogo} itemsList={filterOnlyType(obsItemsList, RawObsItemTypeImage)} scene={obsScene} teamLogos={teamLogos} updateScene={updateTeamLogosScene}/>
            },
            {
                name: 'Advanced Animations',
                node: <SetupAnimationsComponent updateAnimation={updateAnimation} itemsList={filterOnlyType(obsItemsList, RawObsItemTypeMedia)} data={animations} updateScene={updateAnimationsScene} scene={obsScene}/>
            },
            {
