...
export default function EventPlaceholderComponent({
  params,
}: {
  params: {
    event: Event;
    ...
    <div className="position-relative border border-1 border-primary rounded rounded-3">
      <div className="d-flex flex-column justify-content-center align-items-center p-1">
        ...
        <div
          ...
          id={params.isAuto ? "auto" : ""}
        >
          <div className="d-flex justify-content-evenly align-items-center">
            {params.resetEventPlaceholder != null && (
              <img
                onClick={resetCurrent}
                className="bg-secondary p-1 w-15p rounded rounded-3"
                alt="Delete"
                src="/images/bin_static_sm.png"
              />
            )}
            <div className="w-50p">
              <TextInput
            ...
            <button
              className={`btn btn-sm ${
                params.isSelected ? "btn-primary" : "btn-secondary"
              }`}
              disabled={!isChanged()}
              onClick={onCheck}
            >
              {params.isSelected ? "Stop" : "Copy"}
            </button>
