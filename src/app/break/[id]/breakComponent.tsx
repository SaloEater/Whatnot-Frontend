...
export const BreakComponent: React.FC<BreakComponentProps> = (params) => {
  ...
  function getUsernames() {
    ...
    post(getEndpoints().stream_usernames, body).then(
      (i: GetStreamUsernamesResponse) => {
        setUsernames(
          sortStringsAlphabetically(
            arrayUnique(i.usernames.filter((j) => j != "").map((i) => i.trim()))
          )
        );
      }
    );
  ...
  function refreshEvents() {
    ...
    post(getEndpoints().break_events, eventsBody).then(
      (breakEvents: { events: Event[] }) => {
        ...
        setUsernames((old) => {
          let newU = [...old];
          teamEvents
            .filter((i) => i.customer != "" && i.customer != NoCustomer)
            .forEach((i) => {
  ...
  function updateEvent(event: Event) {
    ...
    post(getEndpoints().event_update, event).then((response) => {
      if (response.success) {
        let moveBody = {
          id: event.id,
        ...
        post(getEndpoints().event_move, moveBody).then((_) => {
  ...
  function resetEvent(event: Event) {
    ...
    post(getEndpoints().event_update, updateEventBody).then((response) => {
      if (response.success) {
        let moveBody = {
          id: event.id,
          new_index: 33,
  ...
  function deleteGiveaway(event: Event) {
    ...
    post(getEndpoints().event_delete, updateEventBody).then((response) => {
      if (response.success) {
        setGiveaways((old) => {
  ...
  function saveNewGiveawayCustomer(
    ...
    post(getEndpoints().event_add, event).then((response) => {
  ...
  function moveEvent(event: Event, newIndex: number) {
    ...
    post(getEndpoints().event_move, body).then((response) => {
      if (response.success) {
        refreshEvents();
      }
    });
    ...
    <div className="d-flex m-2 justify-content-evenly">
      <div className="w-75p">
        ...
        <div className="d-flex flex-wrap gap-4 events-container justify-content-center">
          {eventsSorted.map((eventObject, index, arr) => {
            return (
              <EventComponent
                key={eventObject.id}
                event={{ ...eventObject }}
          ...
          <div>
            ...
            <div>
              ...
              <span>
                {giveaways.reduce(
                  (acc, i) =>
                    i.giveaway_type == GiveawayTypeSlab ? acc + 1 : acc,
                  0
                )}
              </span>
            </div>
      ...
      <div className="w-15p">
        <ToolsTabComponent
          events={events}
          changeIndex={moveEvent}
          streamId={params.breakObject.day_id}
          breakO={params.breakObject}
          highBidTeam={params.breakObject.high_bid_team}
          giveawayTeam={params.breakObject.giveaway_team}
          updateHighBidFloor={params.updateHighBidFloor}
        />
      </div>
    </div>
  );
