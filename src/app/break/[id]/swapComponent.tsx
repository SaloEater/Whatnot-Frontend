    ...
    <div className="swap-popup-overlay">
      <div className="d-flex align-items-start w-75p">
        <div className="swap-popup d-flex w-100p">
          <div className="d-flex justify-content-between w-90p">
            <div className="h-75v w-50p">
              {sourceTeamsAll.map((event: Event, j, arr) => {
                let itemStyle = isSourceTeam(event)
                  ? "swap-item-taken"
                  : "swap-item-available";
                return (
                  <div
                    key={event.id}
                    className="swap-popup-item d-flex justify-content-center gap-1"
                    onClick={(e) => switchSourceTeam(event)}
                  >
                    <div
                      className={"border-dashed w-75p " + itemStyle}
                    >{`${event.team}`}</div>
            ...
            <div className="h-75v w-50p">
              {sourceTeams.length > 0 &&
                targetTeamsAll.map((event: Event, j, arr) => {
                  let itemStyle = isTargetTeam(event)
                    ? "swap-item-taken"
                    : "swap-item-available";
                  return (
                    <div
                      key={event.id}
                      className="swap-popup-item d-flex justify-content-center gap-1"
                      onClick={(e) => switchTargetTeam(event)}
                    >
                      <div
                        className={"border-dashed w-75p " + itemStyle}
                      >{`${event.team}`}</div>
          ...
          {sourceTeams.length > 0 && targetTeams.length > 0 && (
            <div>
              <button className="btn btn-primary" onClick={swapTeams}>
                Swap
              </button>
            </div>
          )}
        </div>
        <button className="swap-close-button" onClick={params.onClose}>
          Close
        </button>
      </div>
    </div>
